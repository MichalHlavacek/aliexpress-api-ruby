module AliExpress
  class Affiliate < Base
    ...
    SORTS   = %w(orignalPriceUp orignalPriceDown volumeUp volumeDown sellerRateUp sellerRateDown).freeze
    FIELDS  = %w(productId productTitle productUrl imageUrl allImageUrls originalPrice salePrice localPrice discount volume validTime storeName storeUrl).freeze
    ...
    ERRORS  = {
      ...
      20030090 => 'Tracking ID input parameter error.', # Buggy.
    ...
    class << self
      def search(query: nil, filters: {}, sort: 'orignalPriceUp', page: nil, per_page: 40, fields: FIELDS)
        response = get(
